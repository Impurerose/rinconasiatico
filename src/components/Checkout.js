...
function Checkout() {
  const { cartItems, addToCart, decreaseQuantity, getCartTotal } =
    useShoppingCart();
  ...
  const generateOrderMessage = () => {
    ...
    message += `\nTotal a pagar: $${new Intl.NumberFormat("es-CL").format(
      getCartTotal()
    )}`;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-4xl text-merienda text-primary font-semibold mb-8">
        Tu Pedido
      </h1>
      ...
      {cartItems.length === 0 ? (
        <div className="text-center py-8">
          ...
          <Link
            to="/"
            className="text-lg mt-4 px-10 text-merienda py-4 font-semibold mx-auto block rounded-3xl w-fit transition duration-700 hover:bg-[#a19150] bg-[#6c722f] text-white"
          >
            Volver a la tienda
          </Link>
        </div>
        ...
        <>
          ...
          <div className="mt-8 bg-white p-6 rounded-lg shadow-md">
            <div className="flex justify-between items-center text-xl font-semibold text-primary">
              <span>Total:</span>
              <span>
                ${new Intl.NumberFormat("es-CL").format(getCartTotal())}
              </span>
            </div>

            <div className="mt-10 flex flex-col space-y-4 lg:flex-row lg:space-y-0 lg:space-x-4 text-center items-center justify-end">
              <Link
                to="/"
                className="w-full lg:max-w-sm text-lg px-8 py-3 border-2 border-[#6c722f] text-[#6c722f] font-medium rounded-3xl transition-colors duration-300 hover:bg-[#f3e0bc]"
              >
                Seguir comprando
              </Link>
              <a
                ...
                className="w-full lg:max-w-sm text-lg px-8 py-3 bg-[#6c722f] text-white font-medium rounded-3xl transition-colors duration-300 hover:bg-[#8a913b] flex items-center justify-center"
              >
